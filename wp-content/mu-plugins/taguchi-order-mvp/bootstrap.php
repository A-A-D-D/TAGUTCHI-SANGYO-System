...
final class Taguchi_Order_MVP {
    ...
    private static function normalize_submission( array $posted_data ): array {
        ...
        if ( isset( $normalized['site_select'] ) && ctype_digit( $normalized['site_select'] ) ) {
            $site_post = get_post( (int) $normalized['site_select'] );
            if ( $site_post ) {
                $normalized['site_select'] = get_the_title( $site_post );
            }
        }
    ...
    private static function create_pdf( string $receipt_id, array $data ): string {
        ...
        $mpdf = new \\Mpdf\\Mpdf(
            array(
                ...
                'margin_bottom' => 14,
                'margin_left'   => 14,
                'fontDir'       => array_merge( $font_dirs, array( $font_dir ) ),
                'fontdata'      => $font_data,
